...
describe Concerns::Querying do
  ...
  describe ".find_with_any!" do
    it "should find a record using the secondary key" do
      ...
      expect(MockQueryingOtherModel).to receive(:find_by!).with(handle: subject.handle).and_call_original
    ...
    it "should find a record using the secondary query" do
      subject = FactoryGirl.create(:model)
      expect(MockQueryingModel).to receive(:find_by!).with("name = :id", {id: subject.name}).and_call_original
